...
import { AlertCircle, CheckCircle2, Clock, MapPin, Navigation, RefreshCcw, Loader2, Search, X, Cloud, Sun, CloudRain, Trash2 } from 'lucide-react';
...
// 날씨 캐시 키 & TTL (10분)
const WEATHER_CACHE_KEY = 'cc_weather_cache';
...
export default function DashboardPage() {
  ...
  // 글로벌 Script 태그를 통해 로드되므로 상태 관리 대신 window 객체 확인
  ...
  useEffect(() => {
    async function fetchWeather(lat: number, lon: number) {
      ...
      if (json.main) {
        ...
        setWeather(data);
        localStorage.setItem(WEATHER_CACHE_KEY, JSON.stringify({ data, lat, lon, ts: Date.now() }));
      }
    }
    ...
    async function initWeather() {
      try {
        ...
        if (cached) {
          ...
          if (Date.now() - ts < WEATHER_TTL_MS) {
            setWeather(cachedData);
            return;
          }
        }

        // 브라우저 Geolocation으로 현재 위치 취득
        ...
        // 위치 거부 또는 실패 시 서울 기본값
        try { await fetchWeather(37.5665, 126.9780); } catch (err) { console.error('날씨 정보 연동 실패:', err); }
      }
    }
    initWeather();
  }, []);
  ...
  // 통계 카운트 — 한 번만 순회
  const { rejectedCount, inProgressCount, completedCount } = useMemo(() => {
    if (!projects) return { rejectedCount: 0, inProgressCount: 0, completedCount: 0 };
    let rc = 0, ic = 0, cc = 0;
    for (const p of projects) {
      if (p.status === 'REJECTED')    rc++;
      else if (p.status === 'IN_PROGRESS') ic++;
      else if (p.status === 'COMPLETED')   cc++;
    }
    return { rejectedCount: rc, inProgressCount: ic, completedCount: cc };
  }, [projects]);

  const rejectedProjects   = useMemo(() => filtered.filter(p => p.status === 'REJECTED'),    [filtered]);
  const inProgressProjects = useMemo(() => filtered.filter(p => p.status === 'IN_PROGRESS'), [filtered]);

  // 안정적인 콜백 — 자식 컴포넌트 재렌더 방지
  const handleRestart  = useCallback((id: string) => updateStatus({ id, status: 'IN_PROGRESS' }), [updateStatus]);
  const handleComplete = useCallback((id: string) => updateStatus({ id, status: 'COMPLETED' }),   [updateStatus]);

  const handleDeleteProject = useCallback(async (id: string, name: string) => {
    if (!confirm(`「${name}」 작업장을 삭제하시겠습니까?\n\n사진, AR 스팟, 정산 정보가 모두 영구 삭제됩니다.`)) return;
    try {
  ...
  return (
    <div className="p-8 min-h-screen">
      ...
      {/* 헤더 */}
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-0 mb-10">
        <div>
        ...
        <div className="flex items-center gap-3">
          ...
          {weather && (
            <div className="px-4 py-2 rounded-full flex items-center gap-2" style={{ background: 'rgba(255,255,255,0.85)', border: '1px solid rgba(14,165,233,0.2)', boxShadow: '0 2px 8px rgba(14,165,233,0.1)' }}>
              ...
              <span className="text-sm font-semibold" style={{ color: '#334155' }}>{weather.city && `${weather.city} `}{weather.temp}°C {weather.desc}</span>
            </div>
          )}
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                {rejectedProjects.map(project => (
                  <ProjectRow key={project.id} project={project} onDetail={() => setSelectedProject(project)} onRestart={handleRestart} onDelete={handleDeleteProject} isPending={isPending} variant="rejected" />
                ))}
              </div>
            )}
          </section>
            ...
            {isLoading ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                {inProgressProjects.map(project => (
                  <ProjectRow key={project.id} project={project} onDetail={() => setSelectedProject(project)} onRestart={handleComplete} isPending={isPending} variant="inprogress" />
                ))}
              </div>
            )}
          </section>
...
const ProjectRow = memo(function ProjectRow({ project, onDetail, onRestart, onDelete, isPending, variant }: {
  project: ProjectData;
  onDetail: () => void;
  ...
  return (
    <div
      className="p-5 rounded-2xl flex justify-between items-center transition-all hover:-translate-y-0.5"
    ...
    >
        ...
        <div>
          ...
          <p className="text-sm font-medium" style={{ color: '#64748b' }}>
            ...
            {isRejected && project.rejectReason && (
              <> | <span className="font-semibold" style={{ color: accentColor }}>{project.rejectReason}</span>{project.timeElapsed && ` (${project.timeElapsed})`}</>
            )}
          </p>
        </div>
      </div>
      <div className="flex gap-2 items-center">
        ...
        >상세 보기 및 정산</button>
        <button onClick={() => onRestart(project.id)} disabled={isPending}
