...
export default function UsersPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, async (snapshot) => {
      if (snapshot.empty) {
        try { await seedUsersData(); } catch { setIsLoading(false); }
        return;
      }
    ...
    <div className="p-8 min-h-screen flex flex-col">
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        {[
          ...
          { label: '관리자',        value: users.reduce((n, u) => n + (u.role === 'MASTER' ? 1 : 0), 0),             color: 'text-red-400' },
          { label: '사무실무자',    value: users.reduce((n, u) => n + (u.role === 'ADMIN'  ? 1 : 0), 0),             color: 'text-blue-400' },
          { label: '현장작업자',    value: users.reduce((n, u) => n + (u.role === 'WORKER' ? 1 : 0), 0),             color: 'text-emerald-400' },
        ].map((s, i) => (
