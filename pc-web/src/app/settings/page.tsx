...
import { Shield, Key, Database, RefreshCcw, Smartphone, ToggleLeft, ToggleRight, CheckCircle2, Download, Upload, FolderOpen } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  return (
    <div className="p-8 min-h-screen flex flex-col max-w-6xl">
      <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-4 md:gap-0">
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* 4. 회사 서류 관리 (모바일 팩스 연동) */}
        <section className="bg-slate-800/40 border border-slate-700 rounded-2xl p-8 lg:col-span-2">
          <div className="flex items-center gap-3 mb-4">
            <FolderOpen className="text-violet-500" size={24} />
            <h2 className="text-xl font-bold text-white">회사 필수 서류 관리</h2>
          </div>
          <p className="text-slate-400 text-sm mb-6">
            사업자등록증, 인증서, 영업필증 등 현장에서 자주 요구되는 서류를 등록합니다.
            등록된 서류는 모바일 앱의 [현장 즉시 팩스 발송] 화면에 자동으로 동기화됩니다.
          </p>
          <Link
            href="/settings/documents"
            className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-white rounded-xl font-bold shadow-lg shadow-violet-500/20 transition-all"
          >
            <FolderOpen size={18} /> 서류 관리 페이지로 이동
          </Link>
        </section>
